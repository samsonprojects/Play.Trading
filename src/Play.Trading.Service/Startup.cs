    ...
    public class Startup
    {
        ...
        private void AddMassTransit(IServiceCollection services)
        {
            services.AddMassTransit(configure =>
            {
                ...
                configure.AddConsumers(Assembly.GetEntryAssembly()); //registers all consumers in project with masstransit
                configure.AddSagaStateMachine<PurchaseStateMachine, PurchaseState>(
                    sagaConfigurator =>
                    {
                        sagaConfigurator.UseInMemoryOutbox(); // ensures message is not sent until updates made to state machine, ensures consistent behavior
                    }
                )
                .MongoDbRepository(r =>
                {
